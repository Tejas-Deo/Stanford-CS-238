...
DISCOUNT = 0.99
epsilon = 1
EPSILON_DECAY = 1 #0.95 ## 0.9975 99975
MIN_EPSILON = 0.01
...
MODEL_PATH = 'models/Xception____-6.00max_-189.00avg_-258.00min__1677578929.model'
...
class CarEnv:
    ...
    def reset(self):
        ...
        r = np.random.choice((1,3,5))
        initial_pos = curves[self.curves][r]
        self.via = r+1
        ...
        return [self.front_camera, 0,0, initial_pos[0], initial_pos[1]]
    ...
    def step(self, action, current_state):
        ...
        if action == 3:
            #self.vehicle.apply_control(carla.VehicleControl(throttle=1.0, steer=1*self.STEER_AMT))
            self.vehicle.apply_control(carla.VehicleControl(brake = 0.1, throttle=0, steer=0*self.STEER_AMT))
            action_throttle = 0
            action_steer = 0
            action_break = 0.1
        ...
        # to keep the car aligned with the road
        if abs(phi)<10:
            reward += 1
        else:
            reward -= 1
            
        # to reward the car for decreasing "phi"
        if abs(phi)<abs(current_state[1]):
            reward += 1
        else:
            reward -= 1
            
        #to reward the car for taking an action that leads to decreasing "phi"
        if phi<0:
            if action_steer < 0:
                reward += 1
            elif action_steer > 0:
        ...
        # to reward the car for its traveling distance
        if kmh<10:
            reward += -3
        else:
            reward += 3
        ...
        print(phi)
        #if abs(orientation_diff%360 -360*(orientation_diff%360>180))>100:
            #done = True
            #reward = -200
        ...
        return [self.front_camera, kmh, phi, pos.x, pos.y], reward, done, waypoint
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        model3 = Sequential()
        model3.add(Dense(32, input_shape=(14,), activation='relu', name='dense1'))
    ...
    def train(self):
        ...
        current_data = np.array([[transition[0][i] for i in range(1,5)] for transition in minibatch])
        ...
        new_current_data = np.array([[transition[3][i] for i in range(1,5)] for transition in minibatch])
        with self.graph.as_default():
        ...
        for index, (current_state, action, reward, new_state, done) in enumerate(minibatch):
            ...
            X_data.append([current_state[i] for i in range(1,5)])
            y.append(current_qs)
    ...
    def train_in_loop(self):
        ...
        X2 = np.random.uniform(size=(1, 4)).astype(np.float32)
...
if __name__ == '__main__':
    ...
    agent.get_qs([np.ones((env.im_height, env.im_width, 3)), 0,0, curves[env.curves][1][0],curves[env.curves][1][1]])
